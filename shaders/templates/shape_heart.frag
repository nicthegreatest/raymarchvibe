...
vec2 map( vec3 q )
{
    ...
    float r = 15.0;
    q.y -= r;
    float ani = pow( 0.5+0.5*sin(6.28318*iTime + q.y/25.0), 4.0 ) + (iAudioAmp * u_audio_reactivity);
...
vec3 render( in vec2 p )
{
    ...
    if( t>0.0 )
    {
        ...
        float occ = calcAO( pos, nor ); occ = pow(occ, u_ao_strength);

        if( res.y<1.5 ) // heart
        {
            col = u_heartColor;
            col = col*0.72 + 0.2*fre*u_glowColor;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    col *= 0.2 + 0.8*pow(16.0*q.x*q.y*(1.0-q.x)*(1.0-q.y), u_vignette);
