...
/*
 ...
 - You can find a GPU version of this library in the 'gpu' branch.
 */
...
// FFT argument: std::vector<std::complex<T>>
// T is the precision type (float or double)

// FFT direction specifier
enum class fft_dir {
    DIR_FWD = +1, // forward direction
    DIR_BWD = -1  // backward direction
};

// FFT routines
template<typename T>
std::vector<std::complex<T>> fft1d(
    const std::vector<std::complex<T>> &xi,
    const fft_dir &dir
);

template<typename T>
std::vector<std::complex<T>> fft2d(
    const std::vector<std::complex<T>> &xi,
    const fft_dir &dir
);

template<typename T>
std::vector<std::complex<T>> fft3d(
    const std::vector<std::complex<T>> &xi,
    const fft_dir &dir
);
...
std::vector<std::complex<T>> fft2d(
    ...
    T nrm = T(1) / T(cnt);
    std::vector<std::complex<T>> xo(cnt2);
...
std::vector<std::complex<T>> fft3d(
    ...
    std::vector<std::complex<T>> xo(cnt3);
