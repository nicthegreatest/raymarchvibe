...
#include "Utils.h" // <<< Ensure this include is present

#include <sstream>
#include <regex>
#include <algorithm> 
#include <iomanip>   
#include <iostream> // For potential std::cerr debugging
...
// <<< REMOVE the old ShaderParser::trim definition from here if it exists >>>
// std::string ShaderParser::trim(const std::string& str) { /* ... old implementation ... */ }
...
TextEditor::ErrorMarkers ShaderParser::ParseGlslErrorLog(const std::string& log) {
    ...
    while (std::getline(iss, line)) {
        ...
        if (std::regex_search(line, match, errorRegex1)) { 
            ...
            errorMessage = Utils::Trim(match[3].str()); // <<< CORRECTED
...
void ShaderParser::ScanAndPrepareDefineControls(const char* shaderCode) {
    ...
    while (std::getline(iss, line)) {
        currentLineNumber++;
        std::string trimmedLine = Utils::Trim(line); // <<< CORRECTED
        ...
        if (std::regex_match(trimmedLine, match, defineRegex)) {
            ...
            if (match[3].matched) { 
                dc.originalValueString = Utils::Trim(match[3].str()); // <<< CORRECTED
...
std::string ShaderParser::ToggleDefineInString(const std::string& shaderCode, const std::string& defineName, bool enable, const std::string& originalValue) {
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        std::string trimmedLine = Utils::Trim(lines[i]); // <<< CORRECTED
        ...
        if (std::regex_match(trimmedLine, match, defineLineRegex)) {
            ...
            if (enable && !currentlyEnabled) { 
                ...
                if (commentPos != std::string::npos) {
                    ...
                    lines[i] = Utils::Trim(lines[i]); // <<< CORRECTED
                }
    ...
    while (std::getline(iss, line)) {
        ...
        if (std::regex_match(trimmedLine, match, uniformRegex)) {
            ...
            try {
                ...
                // std::cerr << "JSON parse error for uniform " << nameStr << ": " << e.what() << std::endl;
            }
        }
    }
...
void ShaderParser::ParseConstValueString(const std::string& valueStr, ConstVariableControl& control) {
    std::string valToParse = Utils::Trim(valueStr); // <<< CORRECTED
    ...
    try {
        if (control.glslType == "float") {
            ...
            if (std::regex_match(valToParse, vecMatch, vecRegex)) {
                std::string argsStr = Utils::Trim(vecMatch[2].str()); // <<< CORRECTED
                std::stringstream ss(argsStr);
                std::string segment;
                std::vector<float> components;
                while(std::getline(ss, segment, ',')) {
                    components.push_back(std::stof(Utils::Trim(segment))); // <<< CORRECTED
                }
        ...
        // std::cerr << "Failed to parse const value: " << valueStr << " for " << control.name << " - " << e.what() << std::endl;
    }
}
...
void ShaderParser::ScanAndPrepareConstControls(const std::string& shaderCode) {
    ...
    while (std::getline(iss, line)) {
        ...
        if (std::regex_search(trimmedLine, match, constRegex)) {
            ...
            control.originalValueString = Utils::Trim(match[3].str()); // <<< CORRECTED
